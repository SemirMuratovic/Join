...
async function loadContactsAndTasksUserOrGuest() {
  ...
  if (userLogin == "true") {
    ...
    if (user) {
      allContacts = JSON.parse(await getItem(`${user}_contacts`));
...
async function sendContactsToServer() {
  ...
  if (userLogin == "true") {
    let user = await loadUserData();
    if (user) {
      await setItem(`${user}_contacts`, JSON.stringify(allContacts));
      await setItem(`${user}_tasks`, JSON.stringify(tasks));
    }
  } else {
...
function updateCurrentContactsInTasks() {
  for (let i = 0; i < tasks.length; i++) {
    const updatedArray = tasks[i]["current_contacts"].filter((obj1) =>
      allContacts.some((obj2) => obj2.name === obj1.name && obj2.e_mail === obj1.e_mail && obj2.phone === obj1.phone && obj2.color === obj1.color)
    );
