...
async function getTasksFromServer() {
  ...
  if (user) {
    allTasks = JSON.parse(await getItem(`${user.id}_tasks`));
  } else {
...
async function sendCategoriesToServer() {
  let user = await checkUser();
  if (user) {
    await setItem(`${user.id}_categories`, JSON.stringify(categories));
  } else {
...
async function sendNewTaskToServer() {
  ...
  if (allTasks.length > 0) {
    if (user) {
      await setItem(`${user.id}_tasks`, JSON.stringify(allTasks));
      clearAllTaskContainers();
      renderAlert("alert_container", "alert_content", "A new task has successfully been created and added to your board.");
    } else {
      await setItem("guestTasks", JSON.stringify(allTasks));
      clearAllTaskContainers();
      renderAlert("alert_container", "alert_content", "A new task has successfully been created and added to the board. You will be directed to the board.");
    }
