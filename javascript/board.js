...
function renderTasksOnBoard(i, oneTask, status) {
  ...
  listStatus.forEach((targetStatus) => {
    const tasksWithTargetStatus = tasks.filter((task) => task.status === targetStatus);
    renderTaskList(targetStatus, tasksWithTargetStatus);
    if (status === targetStatus) {
      document.getElementById(status).innerHTML += generateToDoHTML(i, oneTask, newTruncatedSentence, completedSubtasksInPercent);
    }
  });
}
...
function renderTaskList(targetStatus, tasksWithTargetStatus) {
  ...
  if (tasksWithTargetStatus.length === 0) {
    listElement.innerHTML = `<div class="empty-list-message">No tasks ${replaceStatusText(targetStatus)}</div>`;
  }
}
...
function updateCompletedTasks(i, oneTask) {
  const completedSubtasksBox = document.getElementById(`label_for_progress_bar_${i}`);
...
function renderContactsOnOutsideCard(i, oneTask) {
  ...
  if (oneTask.current_contacts.length <= 0) {
    taskContactContainer.innerHTML = '<div class="not-assigned">No contacts assigned</>';
  } else {
...
function calculateNumberOfVisibleContacts(i, oneTask, taskContactContainer) {
  ...
  while (calculateTotalWidthOfContacts(visibleContacts) > maxWidth && visibleContacts.length > 1) {
    hiddenContactsCount++;
    visibleContacts.pop();
  }
 ...
 */
function showVisibleContactsAndOverflowIndicator(i, visibleContacts, hiddenContactsCount, taskContactContainer) {
  ...
  if (hiddenContactsCount > 0) {
    const overflowIndicatorHTML = generateOverflowIndicatorOutsideCardHTML(hiddenContactsCount);
...
function renderContactsInsideCard(i, oneTask) {
  const detailTaskContactNameContainer = document.getElementById(`detail_contacts_name_${i}`);
  ...
  for (let j = 0; j < oneTask.current_contacts.length; j++) {
    ...
    detailTaskContactNameContainer.innerHTML += generateDetailTaskContactNamesHTML(i, j, oneContact);
...
function renderSubtasksDetailView(i, oneTask) {
  const subtaskContainer = document.getElementById(`detail_subtasks_wrapper_${i}`);
 ...
 * next to each task's subtask.
 */
function markCheckboxesAccordingToStatus(checkedStatus, individualSubtaskCheckbox) {
